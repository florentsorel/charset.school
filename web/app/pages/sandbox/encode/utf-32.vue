...
useHead({
  title: () => t('sandbox.encode_utf32.title')
})
...
type Utf32EncodeSandboxResponse = {
  codepoint: number
...
type EncodeOutcome
  = | { ok: true, response: Utf32EncodeSandboxResponse }
    | { ok: false, error: string }

const { data: outcome } = await useAsyncData<EncodeOutcome | null>(
  'sandbox-utf32-encode',
  async (): Promise<EncodeOutcome> => {
    try {
      const result = await $api<Utf32EncodeSandboxResponse>(
        `/sandbox/encode/utf-32?input=${encodeURIComponent(debouncedInput.value)}&endian=${endian.value}`
      )
  ...
  <main class="min-w-0">
    <header class="mb-10">
      ...
      <h1 class="text-3xl font-medium leading-tight tracking-tight mb-2">
        {{ t('sandbox.encode_utf32.title') }}
      </h1>
      <p class="text-sm text-mute leading-relaxed max-w-xl">
        {{ t('sandbox.encode_utf32.subtitle') }}
      </p>
    </header>
      ...
      <section>
        ...
        <ol class="flex flex-col gap-0">
          ...
          >
            ...
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.encode_utf32.step.endianness') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc :text="t(`sandbox.encode_utf32.step.endianness_desc.${endianStep.value}`)" />
              </p>
          ...
          >
            ...
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.encode_utf32.step.binary') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc :text="t('sandbox.encode_utf32.step.binary_desc', { cp: response.codepointLabel })" />
              </p>
              <div class="surface px-5 py-3 inline-block">
          ...
          <li class="flex gap-5">
            ...
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.encode_utf32.step.hex') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc :text="t('sandbox.encode_utf32.step.hex_desc')" />
              </p>
              <div class="surface px-5 py-3 inline-block">
