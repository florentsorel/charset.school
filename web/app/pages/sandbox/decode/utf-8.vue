...
useHead({
  title: () => t('sandbox.decode_utf8.title')
})
...
type Utf8DecodeSandboxResponse = {
  bytes: number[]
  codepoint: number
...
// Initial input from the `?bytes=` query param, shareable between users.
// Falls back to `C3 A9` (= é) so the page demonstrates something on first
// load. No space-recovery hack needed here because hex byte input cannot
// contain a literal `+`.
const route = useRoute()
const router = useRouter()
const initialBytes = (() => {
  const q = route.query.bytes
  return typeof q === 'string' && q.length > 0 ? q : 'C3 A9'
})()
...
const errorMessage = computed(() => {
  ...
  return t(`sandbox.decode_utf8.errors.${apiError.value.reason}`)
})
...
// Same bit colouring scheme as the encode page: marker on the leader,
// cont on continuation bytes, payload on the data bits.
  ...
  <main class="min-w-0">
    <header class="mb-10">
      ...
      <h1 class="text-3xl font-medium leading-tight tracking-tight mb-2">
        {{ t('sandbox.decode_utf8.title') }}
      </h1>
      <p class="text-sm text-mute leading-relaxed max-w-xl">
        {{ t('sandbox.decode_utf8.subtitle') }}
      </p>
    </header>
    ...
    <section class="surface-subtle p-5 sm:p-6 mb-8">
      <div class="field">
        <label
          ...
          for="sb-bytes"
        >
          {{ t('sandbox.decode_utf8.input_label') }}
        </label>
        <input
          id="sb-bytes"
          v-model="rawBytes"
          ...
          autocomplete="off"
          spellcheck="false"
          :placeholder="t('sandbox.decode_utf8.input_placeholder')"
        >
        <p
          v-if="errorMessage"
          class="field-error"
        ...
        <p
          ...
          class="field-help"
        >
          <InlineDesc :text="t('sandbox.decode_utf8.input_help')" />
        </p>
      </div>
    </section>
    ...
    <template v-if="response">
      <section class="section-card mb-6">
        ...
        <div class="flex flex-col gap-3">
          ...
          <div>
            <p class="font-mono text-xs uppercase tracking-widest text-faint mb-1.5">
              {{ t('sandbox.decode_utf8.bytes_label') }}
            </p>
            <p class="hex text-xl font-medium">
      ...
      <!-- Step-by-step timeline mirrors the encode page but the flow is
           reversed: identify byte count → strip markers → assemble bits
           → read as code point. -->
      <section>
        ...
        <ol class="flex flex-col gap-0">
          ...
          >
            ...
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.decode_utf8.step.format') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc :text="t(`sandbox.decode_utf8.step.format_desc.${response.bytes.length}`)" />
              </p>
          ...
          >
            ...
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.decode_utf8.step.bit_groups') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc :text="t('sandbox.decode_utf8.step.bit_groups_desc')" />
              </p>
              <div class="surface px-5 py-3 inline-block">
          ...
          >
            <div class="flex flex-col items-center">
              <span class="step-dot">{{ bitGroupsStep ? '03' : '02' }}</span>
              <span class="step-connector" />
            </div>
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.decode_utf8.step.binary') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc :text="t('sandbox.decode_utf8.step.binary_desc', { bits: binaryStep.length })" />
              </p>
              <div class="surface px-5 py-3 inline-block">
          ...
          >
            ...
            <div class="flex-1 pb-6">
              <h3 class="text-sm font-medium mb-2 mt-1">
                {{ t('sandbox.decode_utf8.step.code_point') }}
              </h3>
              <p class="text-sm text-mute mb-3 leading-relaxed">
                <InlineDesc
                  :text="t('sandbox.decode_utf8.step.code_point_desc', {
