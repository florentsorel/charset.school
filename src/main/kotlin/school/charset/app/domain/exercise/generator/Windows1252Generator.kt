...
class Windows1252Generator(
    ...
    override fun generateEncode(level: Int): Exercise.Encode {
        ...
        return Exercise.Encode(codePoint, Encoding.Windows1252, level, steps)
    }
    ...
    override fun generateDecode(level: Int): Exercise.Decode {
        ...
        val steps = bytes.buildDecodeSteps(codePoint)
        return Exercise.Decode(bytes, codePoint, Encoding.Windows1252, level, steps)
    }

    /**
     * Sandbox helper: build the pedagogical steps for encoding [codePoint] in
     * Windows-1252, without going through the level-based exercise flow.
     * Re-uses the encode pipeline of the level-based generator.
     */
    fun buildEncodeStepsFor(codePoint: CodePoint): List<Step> = codePoint.buildEncodeSteps()

    /**
     * Sandbox helper: build the pedagogical steps for decoding [bytes] in
     * Windows-1252. Takes the already-decoded [codePoint] as input (the caller
     * decodes via `Codec.decode` so we don't double-decode here, and so the
     * caller sees the `DecoderException` for unassigned bytes early enough to
     * surface it as an HTTP 422).
     */
    fun buildDecodeStepsFor(bytes: ByteArray, codePoint: CodePoint): List<Step> = bytes.buildDecodeSteps(codePoint)
    ...
    private fun ByteArray.buildDecodeSteps(codePoint: CodePoint): List<Step> {
