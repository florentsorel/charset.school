...
class Utf8GeneratorTest :
    FreeSpec({
        ...
        "1-byte" - {
            ...
            // mechanical busy-work. We collapse to [Format, HexBytes] so the
            // pedagogy stays in the Format step (identity range recognition).
        ...
        "3-byte" - {
            ...
            "U+4E2D (中) -> Format + Binary(16) + UsefulBitCount(16) + BitGroups(4,6,6) + HexBytes" {
                ...
                val exercise = sut.generateEncode(level = 3)
        ...
        "4-byte" - {
            ...
            "U+1F600 (😀) -> Format + Binary(24, padded) + UsefulBitCount(21) + BitGroups(3,6,6,6) + HexBytes" {
                ...
                val exercise = sut.generateEncode(level = 4)
        ...
        "generateDecode" - {
            ...
            "1-byte [0x41] (A) -> Format + CodePointEntry (no Binary, ASCII identity)" {
            ...
            "2-byte [0xC3, 0xA9] (é) -> Format + BitGroups(5,6) + UsefulBitCount(11) + Binary(16, padded) + CodePointEntry" {
            ...
            "3-byte [0xE4, 0xB8, 0xAD] (中) -> BitGroups(4,6,6) + UsefulBitCount(16) + Binary(16) + CodePointEntry" {
                val sut = newDecodeSut(
                    byteArrayOf(0xE4.toByte(), 0xB8.toByte(), 0xAD.toByte()),
                    level = 3,
                )
                val exercise = sut.generateDecode(level = 3)
            ...
            "4-byte [0xF0, 0x9F, 0x98, 0x80] (😀) -> BitGroups(3,6,6,6) + UsefulBitCount(21) + Binary(24, padded) + CodePointEntry" {
                val sut = newDecodeSut(
                    byteArrayOf(0xF0.toByte(), 0x9F.toByte(), 0x98.toByte(), 0x80.toByte()),
                    level = 4,
                )
                val exercise = sut.generateDecode(level = 4)
