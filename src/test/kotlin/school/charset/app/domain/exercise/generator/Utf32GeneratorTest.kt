...
class Utf32GeneratorTest :
    FreeSpec({
        ...
        fun newSut(encoding: Encoding, codePoint: CodePoint, level: Int): Utf32Generator {
            ...
            return Utf32Generator(codec, codePointGenerator, encoding)
        }

        "constructor" - {
            "encoding = Utf32Be is accepted" {
                Utf32Generator(codec, mockk(), Encoding.Utf32Be).encoding shouldBe Encoding.Utf32Be
            }

            "encoding = Utf32Le is accepted" {
                Utf32Generator(codec, mockk(), Encoding.Utf32Le).encoding shouldBe Encoding.Utf32Le
            }

            "throws if encoding is not a UTF-32 variant" {
                val exception = shouldThrow<IllegalArgumentException> {
                    Utf32Generator(codec, mockk(), Encoding.Utf8)
                }
        ...
        "invalid level throws ExerciseGenerationException" - {
            "Be" {
                val sut = Utf32Generator(codec, mockk(), Encoding.Utf32Be)
            ...
            "Le" {
                val sut = Utf32Generator(codec, mockk(), Encoding.Utf32Le)
                val exception = shouldThrow<ExerciseGenerationException> {
                    sut.generate(level = 99, Granularity.Verbose)
                }
